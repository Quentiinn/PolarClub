package com.example.common;

/**
 * Created by quentincourvoisier on 31/01/2018.
 */

public class Constants {

    public static final String HEART_COUNT_MESSAGE = "com.iutbm.example.iutbm.couchot.healthybody.HEART_COUNT_MESSAGE";
    public static final String HEART_COUNT_VALUE = "com.iutbm.example.iutbm.couchot.healthybody.HEART_COUNT_VALUE";

    public static final String TIME_SESSION_MESSAGE = "com.example.common.task.TimerSessionTask.TIME_SESSION_MESSAGE";
    public static final String TIME_SESSION_VALUE = "com.example.common.task.TimerSessionTask.TIME_SESSION_VALUE";

    public static final String DEBUT_SESSION_MESSAGE = "com.example.common.task.DebutSessionTask.DEBUT_SESSION_MESSAGE";
    public static final String DEBUT_SESSION_VALUE = "com.example.common.task.DebutSessionTask.DEBUT_SESSION_VALUE";

    public static final String DB_SESSIONS = "sessions";
    public static final String DB_PARTICIPANTS = "participants";

    public static final String PREF_POLAR = "pref_polar";
    public static final String PREF_USER = "pref_user";
}
